...
class ViT(nn.Module):
    '''
    chw:(channels, height, width) of img
    n_patches:图片被分割的块数，需要能够整除图片尺寸
    n_blocks:使用ViTBlock的个数
    hidden_d:将原始的序列映射到hidden_d维
    n_heads:多头自注意力模块的头的数量
    output_dim:输出维度
    '''
    def __init__(self, chw, n_patches=7, n_blocks=2, hidden_d=8, n_heads=2, output_dim=64):
        # Super constructor
        ...
        self.mlp = nn.Sequential(
            nn.Linear(self.hidden_d, output_dim),
            nn.Softmax(dim=-1)
        )
...
if __name__ == "__main__":
    ...
    vit = ViT((3, 224, 224), n_patches=32, n_blocks=2, hidden_d=8, n_heads=2, output_dim=64)
    # y = resnet50(x)
    y = vit(x)
    print(y.shape)